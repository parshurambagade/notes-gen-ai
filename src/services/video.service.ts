...
export class VideoService {
  static async getVideoDetails(videoId: string): Promise<VideoData> {
    try {
      // Validate input - handle undefined, null, arrays, etc.
      ...
      if (trimmedId.length !== 11) {
        throw new Error(
          `Invalid video ID format. Expected 11 characters, got ${trimmedId.length}`
        );
      }
      ...
      throw new Error(
        "An unexpected error occurred while fetching video details"
      );
    }
  }
}
